jQuery(document).ready(function ($) {
  ...
  // ============================================================
  // AUTO-FILL NAMA TAMU DARI URL PARAMETER (CK2)
  // Membaca ?to=, ?dear=, atau ?kepada= dari URL dan mengisi
  // field nama (#author) secara otomatis, serta menyembunyikan
  // notif "Khusus untuk tamu undangan" yang tampil by default.
  // Menggantikan kebutuhan script manual di setiap template.
  // Support: URL encoding penuh (%26 = &, %27 = ', + = spasi)
  // ============================================================
  (function () {
    var urlParams = new URLSearchParams(window.location.search);
    var namaTamu = urlParams.get('to') || urlParams.get('dear') || urlParams.get('kepada');
    if (namaTamu) {
      // Decode URL encoding: %26 → &, %27 → ', %20/%2B → spasi, dll.
      namaTamu = decodeURIComponent(namaTamu.replace(/\+/g, ' '));

      // Isi field nama #author jika kosong (tidak override input manual)
      var authorInput = document.getElementById('author');
      if (authorInput && !authorInput.value) {
        authorInput.value = namaTamu;
      }

      // Sembunyikan error span nama — tampil by default saat mode
      // comment_by_name=true tapi URL param tersedia
      document.querySelectorAll('.cui-error-info-name').forEach(function (el) {
        el.style.display = 'none';
      });

      // FIX CK2: Gunakan setTimeout sebagai fallback jika ada widget Elementor
      // yang melakukan replace innerHTML setelah document ready
      setTimeout(function() {
        document.querySelectorAll('.cui-error-info-name').forEach(function (el) {
          el.style.display = 'none';
        });
  ...
  // FIX PAGINATION ON LOAD — Inisiasi jPages untuk komentar yang sudah dirender
  // oleh PHP shortcode secara langsung (tanpa klik AJAX).
  // Sebelumnya jPages hanya dipanggil di callback AJAX getComments/insertComment,
  // sehingga jika shortcode merender komentar langsung, pagination tidak pernah muncul.
  if (typeof jQuery.fn.jPages == 'function' && CUI.jpages == 'true') {
    $('ul[id^="cui-container-comment-"]').each(function () {
      var $idList = $(this).attr('id');
      var post_id = $idList.replace('cui-container-comment-', '');
  ...
  $(document).on('submit', '.cui-container-form form', function (event) {
    ...
    if ($content.length < 2) {
      ...
      if ($(this).find('input#author').length) {
        ...
        // FIX — Regex nama diperluas: batas 30 → 100 karakter.
        // Karakter yang DIIZINKAN: huruf, angka, spasi, tanda baca nama umum
        // seperti & ' - . , ( ) dan karakter aman lainnya.
        // Karakter yang DIBLOKIR hanya yang berpotensi XSS: < > " ` \
  ...
  function jPages_CUI(post_id, $numPerPage, $destroy) {
    ...
    if (typeof jQuery.fn.jPages == 'function' && CUI.jpages == 'true') {
      ...
      if (num_comments > $numPerPage) {
        ...
        // FIX CK2 — Tambah .show() agar div.cui-holder-{post_id} tidak tetap display:none
...
jQuery("document").ready(function() {
 ...
 // FIX CK2 — Selector class diperbaiki dari 'li.comment' ke 'li.cui-item-comment'
 // agar jPages bisa menemukan elemen komentar CK2 dan menandai jp-show dengan benar.
 jQuery('.cui-container-comments li.cui-item-comment').addClass('jp-show');
